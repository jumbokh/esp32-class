...
const char* ssid = "jumboap"; //remember change your SSID 
const char* password = "0953313123";// remember change your SSID PW
String myLineNotifyToken = "LINE Token";    //Line Notify Token，You can refer this post to get Line token：https://t.ly/LZwKn
int IRpin=13;
...
void setup() {
  ...
  if (err == ESP_OK) {
    ...
    sensor_t * s = esp_camera_sensor_get();
    int res = 0;
    //res=s->set_brightness(s, 0);//-2 to 2亮度
    //res=s->set_contrast(s, 0);//-2 to 2對比
    //res=s->set_saturation(s, 0);//-2 to 2飽和度
    //res=s->set_special_effect(s, 0);//0=no effect,1 to 6=不同特殊效果
    //res=s->set_whitebal(s, 1);//disabled=0,enabled=1白平衡
    //res=s->set_awb_gain(s, 1);//disabled=0,enabled=1白平衡增益
    //res=s->set_wb_mode(s, 0);//0=default,1 to 4=sunny to home白平衡模式
    //res=s->set_lenc(s, 1);//disabled=0,enabled=1鏡頭校正（較亮）
    //res=s->set_hmirror(s, 0);//disabled=0,enabled=1水平翻轉
    //res=s->set_vflip(s, 1);//disabled=0,enabled=1垂直翻轉
  }  else {
